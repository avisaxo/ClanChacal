...
public class UserAuth : MonoBehaviour {
		...
		public SeleccionUser CreatePlayer;
		...
		void HandleCreateResult(Task<Firebase.Auth.FirebaseUser> authTask, string newDisplayName = null) {

				if (LogTaskCompletion(authTask, "User Creation")) {
						if (auth.CurrentUser != null) {
								Debug.Log(String.Format("User Info: {0}  {1}", auth.CurrentUser.Email,auth.CurrentUser.ProviderId));
								UpdateUserProfile(newDisplayName: newDisplayName);
						}
				}
				dataentry.instance.ProvaiderIDUser = auth.CurrentUser.ProviderId;
				Debug.Log ("Llama a CreateNewPlayer");
				CreateNewPlayer ();
		}
		...
		public void CreateNewPlayer ()
		{
			...
			//CreatePlayer.CreateDB_Player ();
			dataentry.instance.CreateDB_New_Player ();
		}

		public void FindPlayerInstance ()
		{
		...
		void HandleSigninResult(Task<Firebase.Auth.FirebaseUser> authTask) {
				...
				if(LogTaskCompletion(authTask, "Sign-in")){
					LoginGo.SetActive (false);
					SigninGo.SetActive (false);
					screenmanager.Transition ("Jugadores");
				}
		}
